...
// HR command callback — set by main.cpp before jr_ble_init()
static jr_hr_cmd_cb_t g_hr_cmd_cb = nullptr;
...
static void build_packet(jr_packet_v1_t *out)
{
  ...
  if (reset_on_start)
  {
    jump_to_send = (jump_total >= baseline) ? (jump_total - baseline)
                                            : jump_total;
  }
                          ...
                          struct ble_gatt_access_ctxt *ctxt, void *arg)
{
  ...
  case 0x01: // Start streaming
    ...
    portEXIT_CRITICAL(&g_lock);
    ESP_LOGI(TAG, "Streaming START (baseline=%" PRIu32 ")", g_jump_baseline);
    break;

  case 0x00: // Stop streaming
    g_streaming = false;
    ESP_LOGI(TAG, "Streaming STOP");
    break;

  case 0x02: // Start HR task
    ESP_LOGI(TAG, "CMD: start HR task");
    if (g_hr_cmd_cb)
    {
      g_hr_cmd_cb(0x02);
    }
    ...
    {
      ESP_LOGW(TAG, "No HR cmd callback registered");
    }
    break;

  case 0x03: // Stop HR task
    ESP_LOGI(TAG, "CMD: stop HR task");
    if (g_hr_cmd_cb)
    {
      g_hr_cmd_cb(0x03);
    }
