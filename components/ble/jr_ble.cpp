 ...
 *
 * Characteristics:
 * - Control (UUID ...0002): write 0x01 => Start streaming; write 0x00 => Stop streaming
 * - Data    (UUID ...0003): notify/read => jr_packet_v1_t (12 bytes)
 *
 * Notes:
 * - Packet is little-endian (frontend uses DataView.getUint32(..., true)).
 * - "Reset on Start" is implemented using a baseline: transmitted jump_count becomes
 *   (jump_total - baseline_at_start), while internal counter can stay cumulative.
 */
...
static void build_packet(jr_packet_v1_t *out) {
    ...
    if (reset_on_start) {
        ...
        jump_to_send = (jump_total >= baseline) ? (jump_total - baseline) : jump_total;
    }
...
static int gap_event_cb(struct ble_gap_event *event, void *arg) {
    ...
    case BLE_GAP_EVENT_SUBSCRIBE:
        ESP_LOGI(TAG, "Subscribe: conn=%d attr=%d notify=%d indicate=%d",
                 ...
                 (int)event->subscribe.attr_handle,
...
static void start_advertising(void) {
    ...
    // requestDevice({filters:[{services:[JR_SERVICE_UUID]}]}) needs the service UUID in advertising.
    ...
    rc = ble_gap_adv_start(g_own_addr_type, NULL, BLE_HS_FOREVER, &adv, gap_event_cb, NULL);
    if (rc != 0) {
...
static void notify_task(void *param) {
    ...
    while (true) {
        if (g_streaming && g_conn_handle != BLE_HS_CONN_HANDLE_NONE && g_data_val_handle != 0) {
            build_packet(&pkt);
